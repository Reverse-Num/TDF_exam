...
\begin{proof}
    ...
    Пусть $s$ --- также натуральный параметр, причём $k, s, \frac{2^k}{s} \to \infty$,  Таблицу разобьём на горизонтальные полосы $A_1, \ldots, A_p$ высоты $s$ (полоса $A_p$ имеет высоту $s' \leqslant s$), $p = \ceil{\frac{2^k}{s}}$.
    ...
    \]
    Подсхема $S_3$ состоит только из дизъюнкторов (получаем $f_{(i,\widetilde{\tau})}^{(1)}$ через СДНФ). Для реализации всех функций для конкретного $i$ необходимо не больше $2^{n-k}$ дизъюнкций (т.\,к. всего $n-k$ переменных), получаем
    \[
    ...
    \begin{multline*}
        ...
        2^{n-k+1} + 2^{k+1} + p2^{n-k} + p2^s s+2p2^s \leqslant \\
